...
def top_k_ranking_with_clip(embeddings,
                            labels,
                            prototypes,
                            prototype_labels,
                            clip_similarity,
    ...
    for i, cs in enumerate(clip_similarity):
        idx = torch.argwhere(prototype_labels==i+1).squeeze()
        # print(idx.shape)
        feature_affinity[:, idx] += clip_scalar * cs
    # exit(0)
