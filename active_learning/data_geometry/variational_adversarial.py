...
from active_learning.data_geometry.vae_model import VAE, Discriminator, AdversarySampler
...
class VAAL(BaseAdversarial):
    ...
    def __init__(self, vae_steps_per=2, discriminator_steps_per=1, beta=1, adversary_param=1, **kwargs):
    ...
    def calculate_representativeness(self, im_score_file, num_samples, already_selected=[], skip=False, **kwargs):
        ...
        labeled_sampler = data.sampler.SubsetRandomSampler(already_selected_indices)
        labeled_dataloader = data.DataLoader(self.all_train_im_files,
                                             sampler=labeled_sampler,
                                             batch_size=self.batch_size,
                                             drop_last=False, pin_memory=True)
        unlabeled_indices = np.setdiff1d(all_indices, already_selected_indices)
        unlabeled_sampler = data.sampler.SubsetRandomSampler(unlabeled_indices)
        unlabeled_dataloader = data.DataLoader(self.all_train_im_files,
    ...
    def train_adv_model(self, vae, discriminator, unlabeled_dataloader, labeled_dataloader):
        ...
        num_steps = self.num_epochs * len(self.all_train_im_files) // self.batch_size
        ...
        for iter_count in range(num_steps):
            ...
            for count in range(self.vae_steps_per):
                ...
                lab_real_preds = lab_real_preds.to(self.gpus)
                unlab_real_preds = unlab_real_preds.to(self.gpus)
            ...
            for count in range(self.discriminator_steps_per):
                ...
                lab_real_preds = lab_real_preds.to(self.gpus)
                unlab_fake_preds = unlab_fake_preds.to(self.gpus)
