...
class DeepBayesianModel(SoftmaxMixin, BaseModel):
    """Deep Baysian Model class"""
    ...
    def __init__(self, ann_type="scribble", data_root="wsl4mis_data/ACDC", ensemble_size=1,
                 ...
                 patch_size=(256,256), seed=0, gpus="0", tag="", T=40, inf_train_output_mode='main', scoring_func="entropy"):
        ...
        self.T = T
        self.inf_train_output_mode = inf_train_output_mode
    ...
    def inf_train_model(self, model_no, snapshot_dir, round_dir, cur_total_oracle_split=0, cur_total_pseudo_split=0):
        ...
        with torch.no_grad(): # All computations inside this context will not track gradients
            for i_batch, sampled_batch in tqdm(enumerate(full_trainloader)):
                ...
                main_seg_repeated, aux_seg1_repeated = model(volume_batch_repeated)
                main_seg_soft_repeated = torch.softmax(main_seg_repeated, dim=1)

                if self.inf_train_output_mode == 'main': # Use only the main output
                    outputs = main_seg_soft_repeated
                elif self.inf_train_output_mode == 'combine': # Average the main and auxiliary outputs
                    aux_seg1_soft_repeated = torch.softmax(aux_seg1_repeated, dim=1)
                    combined_output_repeated = (main_seg_soft_repeated + aux_seg1_soft_repeated) / 2
                    outputs = combined_output_repeated
                else:
                    raise ValueError("Invalid output_mode. Choose 'main' or 'combine'.")

                if self.scoring_func == "entropy":
                    # Average over the T number of forward passes
                    avg_outputs = torch.mean(outputs, dim=0).unsqueeze(0)
                    train_preds[slice_basename] = np.float16(avg_outputs.cpu().detach().numpy())
                elif self.scoring_func == 'bald':
                    # Compute the first term of the BALD equation
                    ...
                    second_term = torch.sum(torch.sum(outputs * torch.log(outputs + 1e-10), dim=0), dim=0) / self.T

                    # Compute the BALD score
                    bald_score = first_term + second_term
                    train_preds[slice_basename] = np.float32(bald_score.cpu().detach().numpy())
    ...
    def __repr__(self):
        ...
        mapping["model_cls"] = "DeepBayesianModel"
