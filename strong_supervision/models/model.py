...
from strong_supervision.utils import losses, val_2D


def train_model(arch, encoder_name, encoder_weights, data_root, snapshot_dir, 
                patch_size=(256, 256), batch_size=16, base_lr=1e-2, max_iterations=600,
                seed=0, gpus="0"):
    ...
    logger_filename = os.path.join(snapshot_dir, "log.txt")
    ...
    model_obj = StrongModel(arch=arch, encoder_name=encoder_name, encoder_weights=encoder_weights)
    ...
    dice_loss = losses.DiceLoss(model_obj.num_classes)
        ...
        for _, sampled_batch in enumerate(train_loader):
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                if performance > best_performance:
                    ...
                    save_best = os.path.join(snapshot_dir + 'log.txt',
            ...
            if iter_num % 3000 == 0:
                save_mode_path = os.path.join(
                    snapshot_dir + 'log.txt', 'iter_' + str(iter_num) + '.pth')
                torch.save(model.state_dict(), save_mode_path)
